...
// From what I can tell, usb.outTransfer is completely broken for transfers
// larger than 64 bytes (even if maxPktSize is adjusted for that endpoint). This
// is a minimal and simplified reimplementation specific to our use cases that
// fixes that bug and is based on the code of USBHost::outTransfer,
// USBHost::SetPipeAddress and USBHost::OutTransfer.
void usbOutTransferChunk(uint32_t addr, uint32_t ep, uint32_t nbytes,
                         uint8_t *data) {
  ...
  if (epInfo->bmSndToggle)
    USB->HOST.HostPipe[epInfo->epAddr].PSTATUSSET.reg =
        USB_HOST_PSTATUSSET_DTGL;
  else
    USB->HOST.HostPipe[epInfo->epAddr].PSTATUSCLR.reg =
        USB_HOST_PSTATUSCLR_DTGL;
...
// This accepts arbitrary sized USB writes and will automatically chunk them
// into writes of size 0x1000 and increment packetsWritten every time a chunk is
// written out.
...
void usbBufferedWriteU32(uint32_t data) { usbBufferedWrite((byte *)&data, 4); }
...
void setup() {
  ...
  // Turn off all LEDs and go to sleep. To launch another payload, press the
  ...
  //   while (true)
  //     ; // Hang to let WDT reset the board
}
